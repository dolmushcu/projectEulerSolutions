...
function horizontal(row, col) {
  let horizontal_product = 1;

  for (let i = col; i < col + 10; i += 3) {
    horizontal_product *= parseInt(grid[row].substr(i, 2));
  }

  if (horizontal_product > product) {
    product = horizontal_product;
  }
}

function vertical(row, col) {
  let vertical_product = 1;

  for (let i = row; i < row + 4; i++) {
    vertical_product *= parseInt(grid[i].substr(col, 2));
  }
  
  if (vertical_product > product) {
    product = vertical_product;
  }
}

function right_diagonal(row, col) {
  let right_diagonal_product = 1;

  for (let i = 0; i < 4; i++) {
    right_diagonal_product *= parseInt(grid[row + i].substr(col + (i * 3), 2));
  }

  if (right_diagonal_product > product) {
    product = right_diagonal_product;
  }
}

function left_diagonal(row, col) {
  let left_diagonal_product = 1;

  for (let i = 0; i < 4; i++) {
    left_diagonal_product *= parseInt(grid[row + i].substr(col - (i * 3), 2));
  }

  if (left_diagonal_product > product) {
    product = left_diagonal_product;
  }
}
